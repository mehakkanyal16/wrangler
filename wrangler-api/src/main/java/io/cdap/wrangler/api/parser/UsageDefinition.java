 ...
 */
 ...
 /**
  * This class {@link UsageDefinition} provides a way for users to register the arguments for UDDs.
  *
 ...
 /**
 * This method formats the token based on its type.
 *
 * @param token the token to format.
 * @return a string representation of the token.
 */
private String formatToken(TokenDefinition token) {
  switch (token.type()) {
    case DIRECTIVE_NAME:
      return token.name();
    case COLUMN_NAME:
    ...
    case TEXT_LIST:
      return token.name() + "[," + token.name() + " ...]*";
    case EXPRESSION:
      return "exp:{<" + token.name() + ">}";
    case PROPERTIES:
      return "prop:{key:value,[key:value]*";
    case RANGES:
      return "start:end=[bool|text|numeric][,start:end=[bool|text|numeric]*";
    case BYTE_SIZE:
      return token.name() + " (e.g., 10KB, 100MB, 1GB)";
    case TIME_DURATION:
      return token.name() + " (e.g., 5s, 2m, 1h)";
    default:
      return token.name(); // Fallback for unhandled types
  }
}
   ...
   public static final class Builder {
     ...
     /**
      * Allows users to define a token with a name, type of the token, and an additional optional
      * label that is used during the creation of the usage for the directive.
      *
      * @param name of the token in the definition of a directive.
      * @param type of the token to be extracted.
